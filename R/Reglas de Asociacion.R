...
# "supp" es el soporte minimo
# "conf" es la confianza minima
# "minlen" es el tamano minimo de itemsets
...
datos <- read.csv("./datasets/lastfm.csv", sep=";",header=TRUE)
...
class(listas)
length(listas)
inspect(head(listas, 6))
#class(datos)
trans4 <- transactions(datos, format = "long", cols = c("user" #TID
                                                      , "artist" #item
                                                        ))
trans4
inspect(head(trans4, 6))
listas
inspect(head(listas, 6))
...
library(dplyr)
df_rules <- DATAFRAME(resultados2)
df_rules %>%
  filter(
    LHS == "{wilco}"
  )
